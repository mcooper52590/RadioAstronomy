...
intensArrUni = np.roll(intensArrUni, xOff, axis=0)
intensArrUni = np.roll(intensArrUni, xOff, axis=1)
...
#axes[0,0].text(0.05, 0.95, 'I(l,m)', transform=axes[0,0].transAxes, fontsize=14,
#        verticalalignment='top', bbox=props)
#axes[0,0].text(0.05, 0.05, 'Map', transform=axes[0,0].transAxes, fontsize=14,
...
#axes[1,0].imshow(np.abs(skyArrFFT), cmap='Greys_r')
...
#axes[1,1].imshow(np.abs(natArrFFTRolled), cmap = 'Greys_r')
...
#
##Panel 5
#axes[0,2].imshow(np.abs(intensArrNat), cmap='Greys_r', vmin = 0, vmax = (np.abs(intensArrNat)).max())
...
#axes[1,2].imshow(sampVis, cmap='Greys_r')
...
fig, (ax1, ax2) = plt.subplots(1, 2, sharex=False, sharey=False, figsize=(9,6)) 
...
ax1.imshow(np.abs(intensArrUni), cmap='Greys_r', vmin = 0, vmax = (np.abs(intensArrUni)).max())
ax1.tick_params(
    axis='both',          
    which='both',      
    bottom=False,      
    ...
    labelbottom=False,
    labelleft=False)  
ax1.set_title('Uniform Weighting')
ax2.imshow(np.abs(intensArrNat), cmap='Greys_r', vmin = 0, vmax = (np.abs(intensArrNat)).max())
ax2.tick_params(
    axis='both',          
    which='both',      
    bottom=False,      
    ...
    labelbottom=False,
    labelleft=False)  
ax2.set_title('Natural Weighting')
fig.tight_layout()
